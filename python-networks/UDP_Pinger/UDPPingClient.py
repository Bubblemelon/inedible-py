...
print("10 packets transmitted, %d received, %d%% packet loss, time %7.2fms" % ( recvNum, percentageLost, (sum(rttList) + (1000*(10-recvNum))) ) )

# prints the min avg max Round Trip Time (rtt)
print("rrt min/avg/max = %5.3f/%5.3f/%5.3f ms\n" % ( min(rttList), (sum(rttList)/len(rttList)) ,max(rttList) ) )
